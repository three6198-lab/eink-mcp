...
2. 点「**连接墨水屏**」，在蓝牙列表里选 `NRF_EPD_xxxx`
   （列表是空的就勾上「列出全部蓝牙设备」再连，详见第八节）
...
| `@template` | 顶部大图标 | 页脚小图标 | 签名 | 着墨 | 语感 |
|---|---|---|---|---|---|
| `claude`（默认） | 红色星芒（Claude 标识） | 小星芒 | 红斜体 `Claude` | 图标与签名红，正文黑 | 正式、干净 |
| `wolf` | 黑色手绘小狼头（比星芒大 40%） | 黑色爪印 | 黑斜体 `Vey` | **除上下两条分隔线是红的，其余全黑** | 轻松、可爱、有涂鸦感 |
...
| 指令 | 说明 |
|---|---|
| `@template` | 卡片模板：`claude`（默认）/ `wolf`。决定顶部大图标、页脚小图标的搭配，以及整套着墨色 |
...
`wolf` 的小狼头与爪印是**从位图线稿自动描摹**出来的矢量轮廓（Marching Squares 提边界 →
RDP 简化，狼头 12 个环 / 183 个点，爪印 5 个环 / 90 个点），同样统一成 viewBox 24×24、
中心 (12,12)，所以缩放换算和星芒共用一套。描摹时按实际显示尺寸**加粗了笔画** ——
...
| 蓝牙列表里搜不到设备 | 名字过滤器没命中，**不是蓝牙坏了**。控制台默认只列 `NRF_EPD` / `EPD` 开头的设备，而 `EPD-nRF5` 系列固件的广播名是 `NRF_EPD_<MAC后两字节>`（如 `NRF_EPD_1A2B`）。勾选「列出全部蓝牙设备」后重新连接，在完整列表里挑那个 `NRF_EPD_xxxx`；勾选状态会记住，下次不用再点 |
