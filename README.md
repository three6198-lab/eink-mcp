...
**上下各一条红色分隔线、顶部一枚 Claude 星芒、居中衬线正文、页脚左边日期右边签名**——
把内容整段以 `[card]` 开头即可：
...
| 指令 | 说明 |
|---|---|
| `@icon` | 顶部图标：`claude`（默认，Claude 星芒标识）/ `star`（几何星芒）/ `heart` / `none`。`sun` 是 `claude` 的旧写法，仍然可用 |
| `@footer` | 页脚。用 `\|` 分左右两栏：左边靠左、右边靠右（右侧会自动带一枚小星芒标识）；不写 `\|` 则整行靠左。**只写 `@footer` 不给值**时，左栏自动填当天日期 |
