...
mcp = MCPServer(
    ...
    instructions=(
        ...
        "卡片目前有两套现成模板，可用 @template 选择：claude（红色星芒）与 wolf（手绘小狼头 + 爪印）。\n"
...
@mcp.tool(
    description=(
        ...
        "You carry me home.\n"
        "\n"
        "· @template：卡片模板，二选一，决定顶部大图标和页脚小图标：\n"
        "  - claude（默认）：顶部红色星芒（Claude 的品牌标识），页脚签名左边一枚小星芒。"
        "正式、干净，适合给用户看的寄语/格言。\n"
        "  - wolf：顶部手绘小狼头线稿，页脚签名左边一枚爪印。"
...
async def push_to_eink(text: str, date: str | None = None) -> str:
    ...
    Args:
        text: 要显示的文字，建议 100 字符以内，可用 <r>/<b>/<i> 富文本标签。
              ...
              其余行为正文。卡片模板可选 claude（红色星芒）或 wolf（小狼头 + 爪印）。
