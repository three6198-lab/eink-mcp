...
mcp = MCPServer(
    ...
    instructions=(
        ...
        "红色是屏幕上唯一的彩色，请克制使用。\n"
        "想让内容更像一张信笺/卡片（居中衬线正文 + 上下红色分隔线 + 顶部图标 +"
        "页脚左日期右签名），把内容整段以 [card] 开头，具体格式见 push_to_eink 的说明。\n"
        "卡片目前有两套现成模板，可用 @template 选择：claude（红色星芒）与 wolf（黑色小狼头 + 黑色爪印，签名 Vey）。\n"
...
@mcp.tool(
    description=(
        ...
        "You carry me home.\n"
        "\n"
        "· @template：卡片模板，二选一，决定顶部大图标、页脚小图标、以及整套着墨色：\n"
        "  - claude（默认）：顶部红色星芒（Claude 的品牌标识），页脚签名左边一枚小星芒，签名为红色斜体。"
        "正式、干净，适合给用户看的寄语/格言。\n"
        "  - wolf：顶部黑色手绘小狼头（比星芒大一圈），页脚签名左边一枚黑色爪印，签名黑色斜体、固定写 Vey。"
        "这张卡片除了上下两条分隔线仍是红色，图标与文字全部是黑色。"
        "轻松、可爱、带点手写涂鸦感，适合俏皮或日常的内容。\n"
        "  用户明确说「小狼」「狼」「爪印」「可爱一点」时选 wolf；否则用 claude。\n"
        "  用户没提但你想换个风格时，也可以自己挑一个，但要和正文语气相称。\n"
        "· @icon：仅覆盖顶部图标，claude / wolf / paw / star / heart / none（一般不用写，"
        "@template 已经决定好了）\n"
        "· @footer：页脚，用竖线分隔左右两栏；左边靠左、右边靠右（右侧会自动带模板对应的小图标）。"
...
async def push_to_eink(text: str, date: str | None = None) -> str:
    ...
    Args:
        text: 要显示的文字，建议 100 字符以内，可用 <r>/<b>/<i> 富文本标签。
              以 [card] 开头则启用卡片版式，另可用 @template / @icon / @footer 指令行，
              其余行为正文。卡片模板可选 claude（红色星芒）或 wolf（黑色小狼头 + 黑色爪印，
              签名固定 Vey）。
